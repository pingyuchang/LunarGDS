# -*- coding: utf-8 -*-
"""
Created on Mon Feb  3 12:28:05 2025
...
# =========================
# Original Code for Apollo16 (Unchanged)
# =========================

# Read Apollo 16 data
kny = pd.read_csv("D:/Sat_MV/APollo16_0427.csv", parse_dates=True, index_col=0, sep=',').iloc[74500:129600]
#kny = pd.read_csv("D:/Sat_MV/APollo16_0527.csv", parse_dates=True, index_col=0, sep=',')
Decimate_factor=1
kny = kny.iloc[::Decimate_factor]  # Downsample: Keep every nth row

# Sampling frequency and time calculations
fs = 3 / Decimate_factor           # Sampling frequency (default: 3 Hz)
...
Coh16_threshold = 0.7  # Coherence threshold for C-response
Q16_threshold = 0.7    # Coherence threshold for Q-response
seg = 1
...
knyX = signal.medfilt(kny['BX'] + 186 , 1)
knyY = signal.medfilt(kny['BY'] + 48 , 1)
knyZ = signal.medfilt(kny['BZ'] - 135 , 1)
...
plt.title("Magnetic Field Variation Over Time (Apollo16)")
...
abgle16 = 164.42 - 29 # for 0427
#abgle16 = 148.52 - 13.5 # for 0527
Theta16 = abgle16 * np.pi / 180
Rphi16 = np.arctan(-1 * np.cos(Theta16) * KnymeanY / KnymeanZ)

# =========================
# New Section: Sliding Windows Analysis for Apollo16
...
# Additionally, we output the estimated Q-based apparent resistivity for each window.
# =========================
...
nperseg_win = min(int(0.9*win_len), win_len)
...
for start in start_indices:
    ...
    X_win = signal.medfilt(window_data['BX'] + 186 + perturb_Mag, 1)
    Y_win = signal.medfilt(window_data['BY'] + 48 + perturb_Mag, 1)
    Z_win = signal.medfilt(window_data['BZ'] - 135 + perturb_Mag, 1)
    ...
    Cr_win = (-1/2) * np.tan(Theta16) * rLunar * median_ratio_win
    ...
    # ---------- Q-based processing ----------
    array121 = np.array([[-np.cos(Rphi16) * np.sin(Theta16), 2 * np.cos(Rphi16) * np.sin(Theta16)],
                          [-np.cos(Rphi16) * np.cos(Theta16), -np.cos(Rphi16) * np.cos(Theta16)],
                          [np.sin(Rphi16), np.sin(Rphi16)]])
...
ZH_csv_filename = "A16_ZH_SlidingWindows_Results.csv"
...
Q_csv_filename = "A16_Q_SlidingWindows_Results.csv"
...
CZH_csv_filename = "A16_ZH_C_Response.csv"
...
CQ_csv_filename = "A16_Q_C_Response.csv"
