...
def process_apollo_data(file_path, station_lat, station_lon, title, offset):
    #data = pd.read_csv(file_path, parse_dates=True, index_col=0, sep=',').iloc[74500:129600] #for 0427
    data = pd.read_csv(file_path, parse_dates=True, index_col=0, sep=',') #for 0527
    fs = 3
    dt = 1 / fs
    endtime = len(data) * dt
    tt = np.arange(0, endtime, dt)

    bx = sp.signal.medfilt(data['BX'] + offset[0], 1)*-1
...
# Constants
#apollo16_file = "D:/Sat_MV/APollo16_0427.csv"
apollo16_file = "D:/Sat_MV/APollo16_0527.csv"
moon_radius = 1737.4

# Process Apollo 12 data
apollo16_data, apollo16_lat_rad, apollo16_lon_rad, apollo16_theta_p, apollo16_phi_p, apollo16_north, apollo16_south = process_apollo_data(
    apollo16_file, -8.9743, 15.5011, "Apollo 16", [186, 48, -135]) # According to Dyal et al. (1973)

# Plot Apollo 12 station and pole positions
fig, ax = plt.subplots(figsize=(8, 8))

# Apollo 15 station position
station16_y = moon_radius * np.sin(apollo16_lat_rad) * np.sin(apollo16_lon_rad)
station16_z = moon_radius * np.cos(apollo16_lat_rad)

# Plot Apollo 12 station and poles
ax.scatter(station16_y, station16_z, color='blue', label='Apollo 16 Station')
ax.scatter(apollo16_north[1], apollo16_north[2], color='red', label='North Pole (AP16)')
ax.scatter(apollo16_south[1], apollo16_south[2], color='black', label='South Pole (AP16)')
...
ax.set_title("Apollo 16 Station and Pole Positions on the Moon (YZ Plane)")
ax.set_xlabel("Y (km)")
ax.set_ylabel("Z (km)")
plt.grid()
...
apollo16_colatitude, apollo16_longitude = station_in_magnetic_coords(apollo16_lat_rad, apollo16_lon_rad, apollo16_north)


print(f"Apollo 16 Magnetic Colatitude: {apollo16_colatitude:.2f}°, Magnetic Longitude: {apollo16_longitude:.2f}°")
