...
def process_apollo_data(file_path, station_lat, station_lon, title, offset):
    ...
    #data = pd.read_csv(file_path, parse_dates=True, index_col=0, sep=',').iloc[242875:286202] #for 0527
    fs = 3
    dt = 1 / fs
    endtime = len(data) * dt
...
# Constants
apollo15_file = "D:/Sat_MV/APollo15_0427.csv"
#apollo15_file = "D:/Sat_MV/APollo15_0527.csv"
moon_radius = 1737.4

# Process Apollo 12 data
apollo15_data, apollo15_lat_rad, apollo15_lon_rad, apollo15_theta_p, apollo15_phi_p, apollo15_north, apollo15_south = process_apollo_data(
    apollo15_file, 26.13239, 3.63330, "Apollo 15", [-3.3, -0.9, 0.2]) # According to Dyal et al. (1973)

# Plot Apollo 12 station and pole positions
fig, ax = plt.subplots(figsize=(8, 8))

# Apollo 15 station position
station15_y = moon_radius * np.sin(apollo15_lat_rad) * np.sin(apollo15_lon_rad)
station15_z = moon_radius * np.cos(apollo15_lat_rad)

# Plot Apollo 12 station and poles
ax.scatter(station15_y, station15_z, color='blue', label='Apollo 15 Station')
ax.scatter(apollo15_north[1], apollo15_north[2], color='red', label='North Pole (AP15)')
ax.scatter(apollo15_south[1], apollo15_south[2], color='black', label='South Pole (AP15)')
...
ax.set_title("Apollo 15 Station and Pole Positions on the Moon (YZ Plane)")
ax.set_xlabel("Y (km)")
ax.set_ylabel("Z (km)")
plt.grid()
...
apollo15_colatitude, apollo15_longitude = station_in_magnetic_coords(apollo15_lat_rad, apollo15_lon_rad, apollo15_north)


print(f"Apollo 15 Magnetic Colatitude: {apollo15_colatitude:.2f}°, Magnetic Longitude: {apollo15_longitude:.2f}°")
