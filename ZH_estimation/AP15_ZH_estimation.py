...
# =========================
# Load Apollo 16 Data
#file_path = "D:/Sat_MV/APollo15_0427.csv"
file_path = "D:/Sat_MV/APollo15_0527.csv"
#file_path = "D:\Sat_MV\Apollo15_0331\Apollo15_05270528.csv"
#data = pd.read_csv(file_path, sep=',').iloc[74500:129600]
data = pd.read_csv(file_path, sep=',').iloc[242875:286202] 
#data = pd.read_csv(file_path, sep=',').iloc[242875:]

"""""..........Read Apollo 15 data................"""
# Rename min/sec
data.rename(columns={'min': 'minute', 'sec': 'second'}, inplace=True)

# Create timestamp
data['timestamp'] = pd.to_datetime(data[['year', 'month', 'day', 'hour', 'minute', 'second']])

# Time since start
tt = (data['timestamp'] - data['timestamp'].iloc[0]).dt.total_seconds()

# Subtract remanent field offsets
offsets = {'BX': 3.3, 'BY': 0.9, 'BZ': -0.2}
for key in offsets:
...
# set timestamp as the index，which make resample effective
data.set_index('timestamp', inplace=True)
# Resample to 1 Hz (1-second average)
data = data.resample('1s').mean()

# Drop NaNs if necessary (optional)
data.dropna(inplace=True)

# Define constants
miu0 = 4 * np.pi / 10**7  # Magnetic permeability
rLunar = 1737  # Moon radius (km)
fs = 1    # New sampling frequency after resampling
...
# =========================
n_segments = 4 # Desired number of time windows
overlap_percentage = 0.7  # 50% overlap
...
for start in start_indices:
    ...
    freqs, Pxx = signal.welch(
        segment_Bx, fs, window='hann', nperseg=len(segment_Bx)//2)
    _, Pzz = signal.welch(segment_Bz, fs, window='hann',
                          nperseg=len(segment_Bx)//2)
    _, Pxz = signal.csd(segment_Bz, segment_Bx, fs,
                        window='hann', nperseg=len(segment_Bx)//2)
...
# Compute C-response
#Theta12 = np.radians(117.43) #for 0427
Theta12 = np.radians(129.51) #for 0527
...
plt.errorbar(1 / filtered_freqs.ravel() / 3600, filtered_Cr12.real.ravel(), 
             yerr= np.abs(filtered_Cr12_err.ravel()), fmt='o', label='Real')
plt.errorbar(1 / filtered_freqs.ravel() / 3600, filtered_Cr12.imag.ravel(), 
             yerr= np.abs(filtered_Cr12_err.ravel()), fmt='ro', label='Imaginary')
...
plt.title('Apollo 15 C-response with Error (Welch’s Method)')
plt.legend()
plt.grid()
plt.show()
...
plt.title('Apollo 15 Apparent Resistivity with Error')
plt.grid()
plt.show()
...
plt.title('Apollo 15 Phase with Error')
plt.grid()
plt.show()
...
save_c_response_results(
    base_filename="Apollo15_results",
